<script>
    ...
    import {addReaction, deleteReaction, editMessage, sendMessage} from "$lib/network/api.ts";
    ...
    function submit() {
        ...
        sendMessage(data.token, contact.id, trimmed, replyingTo?.id).then((message) => {
    ...
    function submitEdit() {
        ...
        editMessage(data.token, contact.id,editingMessage.id, editingMessage.content).then((data) => {
            if (data) {
                messages = messages.map((message) => {
    ...
    function reactEmoji(message, emoji) {
        addReaction(data.token, data.contact.id, message.id, emoji).then((data) => {
            if (data) {
    ...
    function removeReaction(message, reactionId) {
        ...
        if (!reaction) return;
        deleteReaction(data.token, data.contact.id, message.id, reactionId).then((response) => {
